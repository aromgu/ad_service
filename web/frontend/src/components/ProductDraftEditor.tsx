...
/**
 * 프레임 4c — 상품 정보 검토 화면.
 * 초안은 /product/[draftId] 에서 '네이버에 상품 등록'을, 등록된 상품은
 * /store-products/[draftId] 에서 '수정'·'삭제'를 한다.
 */
export function ProductDraftEditor() {
  ...
  // 진행 중인 요청. 그동안 폼과 버튼을 잠근다.
  const [busyAction, setBusyAction] = useState<"register" | "update" | "delete" | null>(null);
  const [saveState, setSaveState] = useState<"idle" | "saving" | "saved" | "error">("idle");
  // '수정' 반영 시각. 버튼 바로 위에 보여 준다 — 맨 위 배너는 스크롤에 가려진다.
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((d) => {
        if (!alive) return;
        // 등록된 상품은 '등록된 상품 관리' 주소에서 연다 — 상단 메뉴가 맞게 켜진다.
        // 예전 링크처럼 주소가 어긋나 있으면 맞는 주소로 옮긴다.
        const base = d.status === "registered" ? "/store-products" : "/product";
        if (!pathname.startsWith(`${base}/`)) {
          router.replace(`${base}/${draftId}`);
          return;
        }
        setDraft(d);
      })
  ...
  /** 아직 저장 안 된 수정을 먼저 서버에 반영한다. 실패하면 옛 값으로 네이버에 보내지 않도록 멈춘다. */
  async function saveNow() {
    if (saveTimer.current) clearTimeout(saveTimer.current);
    await flush();
    if (Object.keys(pending.current).length > 0) {
      throw new ApiError("고친 내용을 저장하지 못했습니다. 잠시 후 다시 시도해 주세요.", 0);
    }
  }
  ...
  async function update() {
    ...
    try {
      await saveNow();
      const saved = await api.updateProduct(draftId);
      setDraft((prev) => (prev ? { ...saved, ...pending.current } : saved));
      setUpdatedAt(new Date());
    } catch (err) {
      setError(err instanceof ApiError ? err.message : "상품 수정에 실패했습니다.");
    } finally {
      setBusyAction(null);
    }
  }
  ...
  async function remove() {
    if (!draft || busyAction) return;
    if (!window.confirm("스마트스토어에서 이 상품을 삭제할까요? 되돌릴 수 없습니다.")) return;
    setBusyAction("delete");
    setError(null);
    try {
      await api.deleteStoreProduct(draftId);
  ...
  const registered = draft.status === "registered";
  // 등록된 상품도 여기서 고친 뒤 '수정'으로 반영한다. 요청이 진행 중일 때만 잠근다.
        ...
        <div className="flex w-full max-w-[940px] flex-col gap-[18px]">
          ...
          <div className="grid grid-cols-[220px_1fr] gap-[18px]">
            <Card className="gap-2.5 p-[18px]">
              ...
              {!locked && draft.image_urls.length > 1 && (
                <button
                  type="button"
                  onClick={() => {
            ...
            <Card className="gap-4">
              ...
              <div className="grid grid-cols-3 gap-3">
                <SmallField label="브랜드" value={draft.brand} disabled={locked} onChange={(v) => patch({ brand: v })} />
                <SmallField label="제조사" value={draft.manufacturer} disabled={locked} onChange={(v) => patch({ manufacturer: v })} />
                <SmallField label="판매자상품코드" value={draft.seller_code} disabled={locked} onChange={(v) => patch({ seller_code: v })} />
              </div>
            </Card>
          </div>
          ...
          <Card className="gap-4">
            ...
            <div className="grid grid-cols-4 gap-3">
              ...
              />
              <NumberField label="할인율" unit="%" value={draft.discount_rate} disabled={locked} onChange={(v) => patch({ discount_rate: v ?? 0 })} />
              <NumberField label="배송비" unit="원" value={draft.shipping_fee} disabled={locked} onChange={(v) => patch({ shipping_fee: v ?? 0 })} />
              <NumberField label="재고수량" value={draft.stock} disabled={locked} onChange={(v) => patch({ stock: v ?? 0 })} />
            </div>
          ...
          <div className="grid grid-cols-2 gap-[18px]">
            <KcCard kc={draft.kc} disabled={locked} onChange={(kc) => patch({ kc })} />
            <TagsCard tags={draft.tags} disabled={locked} onChange={(tags) => patch({ tags })} />
          </div>
          ...
          {updatedAt && (
            <p
              role="status"
              className="m-0 flex items-center gap-2 rounded-[10px] border border-[#2E7D5B] bg-[#132a20] px-4 py-3 text-[13px] text-fg"
            >
              <Check className="size-4 text-success" strokeWidth={3} />
              스마트스토어에 수정 내용을 반영했어요 · {updatedAt.toLocaleTimeString("ko-KR")}
            </p>
          )}
          ...
          {registered ? (
            <div className="grid grid-cols-[1fr_200px] gap-3">
              ...
              >
                {busyAction === "update" ? "수정 중…" : "수정"}
              </button>
              <button
                type="button"
                onClick={remove}
                disabled={locked}
                className="rounded-xl border border-line py-4 text-[14px] text-danger transition-ui hover:border-danger/60 hover:bg-[#2a1a1e] disabled:opacity-40"
              >
            ...
            <button
              ...
              onClick={register}
              disabled={locked}
              className="grad w-full rounded-xl py-4 text-[16px] font-bold text-white transition-ui hover:brightness-110 disabled:opacity-40"
            >
              {busyAction === "register" ? "등록 중…" : "네이버에 상품 등록 →"}
            </button>
          )}
        </div>
      </div>
...
function RegisteredBanner({ draft }: { draft: ProductDraft }) {
  const details = [
    draft.naver_origin_product_no && `상품번호 ${draft.naver_origin_product_no}`,
    draft.registered_at && `${new Date(draft.registered_at).toLocaleString("ko-KR")} 등록`,
  ]
    .filter(Boolean)
    .join(" · ");
  return (
    <div className="flex items-center gap-3 rounded-xl border border-[#2E7D5B] bg-[#132a20] px-[22px] py-[18px]">
      <span className="flex size-[26px] flex-none items-center justify-center rounded-full bg-success text-white">
        <Check className="size-3.5" strokeWidth={3} />
      </span>
      <div className="flex flex-col gap-[3px]">
        <span className="text-[15px] font-bold text-fg">스마트스토어에 등록된 상품입니다</span>
        <span className="text-[12.5px] text-muted">
          {details && `${details} · `}아래에서 고친 뒤 ‘수정’을 누르면 스토어에 반영됩니다
        </span>
      </div>
    </div>
  );
